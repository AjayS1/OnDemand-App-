...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.primaryButton setImage:[UIImage imageNamed:@"checked_checkbox"] forState:UIControlStateNormal];
    [[NSUserDefaults standardUserDefaults]setObject:@"1" forKey:@"isPrimary"];
}
...
- (void)viewDidLayoutSubviews {
    ...
    bgScrollView.contentSize = CGSizeMake(self.view.frame.size.width, self.view.frame.size.height+500);
...
-(IBAction)primaryButtonAction:(UIButton *)sender{
...
- (IBAction)AddCreditCardClicked:(id)sender {
    ...
    else if ([monthTxt.text isEqualToString:@""]) {
...
- (void)sendPayNowApiCall {
    ...
    if (_signatureImageView.image) {
        ...
        NSMutableDictionary *params = [[NSMutableDictionary alloc] initWithObjectsAndKeys:userIdStr,@"UserID",
                                     cardNumberTxt.text,@"CreditCardNumber",
                                     monthStr,@"ExpiryMonth",
                                     yearTxt.text,@"ExpiryYear"
                                     ,securityCodeNumberTxt.text,@"SecurityCode",
                                     nameOfCardTxt.text,@"NameOnCreditCard",
                                     countryNameStr,@"CountryID",
                                     stateOrProvinceTxt.text,@"State",
                                     cityTxt.text,@"City",
                                     addressTxt.text,@"Address",
                                     zipOrPostalCodeTxt.text,@"Zipcode",
                                     amountStr,@"DueAmount",
                                     self.amountPaidForDateID,@"DateID"
                                    
                                     ,nil];
        
        [params setValue:@"2" forKey:@"DateType"];
         [params setValue:imageString forKey:@"Signature"];
         [params setValue:ipAddressStr forKey:@"IpAddress"];
        NSLog(@"Pay Amount Paramtere ====== %@",params);
        [ProgressHUD show:@"Please wait..." Interaction:NO];
        [ServerRequest AFNetworkPostRequestUrl:APIPayNowPaymentCall withParams:params CallBack:^(id responseObject, NSError *error) {
...
//                         if ([buttonTitle isEqualToString:@"OK"]) {
                             ...
                             [[NSUserDefaults standardUserDefaults] synchronize];
                             {
                                 ...
                                 [[NSUserDefaults standardUserDefaults] setObject:valueToSave forKey:@"tryAgainButtonValue"];
                                 [[NSUserDefaults standardUserDefaults] synchronize];
                                 if (sharedInstance.isFromCaneclDateByContractor) {
                                     ...
                                     if (self.isFromCreditCardDeclined) {
                                     ...
                                     rateViewCall.self.dateIdStr = [responseObject objectForKey:@"DateId"];
                                     rateViewCall.isFromCreditCardView = YES;
                                     rateViewCall.isFromDateDetails = NO;
                                     rateViewCall.self.nameStr = [[_dateDetailsDictionaryFromDecline objectForKey:@"EndDateCustomer"] objectForKey:@"UserName"];
                                     rateViewCall.self.imageUrlStr = [NSString stringWithFormat:@"%@",[[_dateDetailsDictionaryFromDecline objectForKey:@"EndDateCustomer"]objectForKey:@"PicUrl"]];
                                     NSString *requestTimeStr = [NSString stringWithFormat:@"%@", [[_dateDetailsDictionaryFromDecline objectForKey:@"EndDateCustomer"]objectForKey:@"EndTime"]];
